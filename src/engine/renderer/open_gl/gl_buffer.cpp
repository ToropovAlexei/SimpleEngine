...
#endif
{
  ...
  GLbitfield flags = 0;
  if (usage == Usage::Dynamic || usage == Usage::Stream) { flags |= GL_DYNAMIC_STORAGE_BIT; }
...
{
#ifndef NDEBUG
  core::assertion(m_size >= offset + size, "Buffer overflow");
  core::assertion(m_usage == Usage::Dynamic || m_usage == Usage::Stream, "Buffer is not dynamic");
#endif
