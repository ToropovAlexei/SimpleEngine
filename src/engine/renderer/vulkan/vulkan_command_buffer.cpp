...
namespace renderer {
  ...
  {
    if (m_commandBuffer) { m_device.getDevice().freeCommandBuffers(m_commandPool, { m_commandBuffer }); }
  }

  void VulkanCommandBuffer::begin(vk::CommandBufferUsageFlags usage)
  {
    core::assertion(!isRecording, "Command buffer is already recording.");
  ...
  {
    core::assertion(isRecording, "Command buffer is not recording.");
    m_commandBuffer.end();
