...
namespace renderer {
  ...
  {
    if (m_shaderModule) { vkDestroyShaderModule(m_device->getDevice(), m_shaderModule, nullptr); }
  }
  ...
  std::vector<char> VulkanShaderModule::readFile(std::string_view filename)
  {
    ...
    if (!file.is_open()) { core::panic("Failed to open shader file!"); }
  ...
  void VulkanShaderModule::createShaderModule(const std::vector<char> &code)
  {
    ...
    checkVkResult(vkCreateShaderModule(m_device->getDevice(), &createInfo, nullptr, &m_shaderModule));
  }
}// namespace renderer
}// namespace engine
